...
from ...common import BaseDynamicHandle, ModelDescriptor, KVConfig
...
class DynamicHandle(BaseDynamicHandle):
    ...
    def get_model_info(self) -> Optional[ModelDescriptor]:
        ...
        any moment.
        """
        info = self._port.call_rpc("getModelInfo", {"specifier": self._specifier, "throwIfNotFound": False})
        if info is None:
            return None
        return info.get("descriptor", None)

    def get_load_config(self) -> KVConfig:
        return self._port.call_rpc("getLoadConfig", {"specifier": self._specifier})
